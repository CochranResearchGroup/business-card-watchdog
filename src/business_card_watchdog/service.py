...
class BusinessCardService:
    ...
    def review_route_readiness(self, run_id: str, *, write: bool = True) -> dict[str, Any]:
        closeout = self.dry_run_closeout(run_id, write=False)
        handoff = self.dry_run_review_handoff(run_id, write=False)
        ...
        for entry in entries:
            ...
            route_ready = (
                ...
                and next_action_name in _SAFE_NEXT_ACTIONS | _EXPLICIT_OPERATOR_ACTIONS | {"none"}
            )
        ...
        if write:
            readiness_path = self.config.runs_dir / run_id / "review_route_readiness.json"
            payload["runtime_artifact_written"] = True
    ...
    def selected_target_approval_boundary(
        ...
        *,
        ...
        approval_packet = self.live_pilot_approval_packet(run_id=run_id, job_id=job_id)
        entries = []
        for entry in list(approval_packet.get("entries") or []):
            ...
            operator_matches = (
                not operator
                or entry_operator == operator
                or entry_operator.startswith("<")
            )
        ...
        if response_to_validate:
            validation = self.validate_live_pilot_operator_response(
                run_id=run_id,
                response=response_to_validate,
            )
            preflight = self.selected_live_target_preflight(
                run_id=run_id,
                response=response_to_validate,
            )
            preview = self.selected_live_target_artifact_preview(
                run_id=run_id,
                response=response_to_validate,
            )
        ...
        payload: dict[str, Any] = {
            ...
            "would_create_selected_live_target": bool(
                preflight and preflight.get("would_create_selected_live_target")
            ),
            "creates_selected_live_target": False,
            "writes_attempted": 0,
            "network_calls_made": 0,
        ...
        if write:
            boundary_path = self.config.runs_dir / run_id / "selected_target_approval_boundary.json"
            payload["runtime_artifact_written"] = True
