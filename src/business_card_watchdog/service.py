...
from .positive_control_resume_readiness_gate import build_positive_control_resume_readiness_gate
...
class BusinessCardService:
    ...
    def operator_dashboard(self, *, run_id: str | None = None) -> dict[str, Any]:
        return build_operator_dashboard(self, run_id=run_id)
    ...
    def agent_review_loop(
        ...
        *,
        ...
        for entry in selected_entries:
            ...
            crop_summary = dict(matrix.get("crop_quality") or {})
            recrop_summary = dict(matrix.get("recrop_proposals") or {})
            ...
            reasons = _agent_review_reasons(
                ...
                recrop_summary=recrop_summary,
                review_notes=notes,
                entry=entry,
            )
            if _crop_quality_needs_review(crop_summary):
                planned_actions.append(
                    {
                        "job_id": job_id,
            ...
            if _review_notes_request_crop_quality(notes) or _crop_quality_needs_review(crop_summary):
                app_intelligence_requests.append(
                    _agent_review_request(
                        job_id=job_id,
                        request_type="assess_crop_quality",
                        reason="accepted review evidence or deterministic crop quality says the crop needs review",
                        qr_summary=qr_summary,
                        orientation_summary=orientation_summary,
                        crop_summary=crop_summary,
                    )
    ...
    def review_route_readiness(self, run_id: str, *, write: bool = True) -> dict[str, Any]:
        return build_review_route_readiness(
            ...
            explicit_operator_actions=_EXPLICIT_OPERATOR_ACTIONS,
            write=write,
        )
    ...
    def lookup_selection_packet(
        ...
        *,
        ...
        return build_lookup_selection_packet(
            self,
            run_id,
            operator=operator,
            sink=sink,
    ...
    def selected_target_approval_boundary(
        self,
        run_id: str,
        *,
        operator: str,
        ...
        return build_selected_target_approval_boundary(
            self,
            run_id,
            operator=operator,
            sink=sink,
    ...
    def selected_target_command_copy_packet(
        ...
        *,
        ...
        return build_selected_target_command_copy_packet(
            self,
            run_id,
            operator=operator,
            response=response,
    ...
    def service_recovery_report(self, *, run_id: str | None = None) -> dict[str, Any]:
        return build_service_recovery_report(self, run_id=run_id)
    ...
    def operator_live_pilot_readiness_packet(
        self,
        *,
        run_id: str,
        sink: str | None = None,
        ...
        return build_operator_live_pilot_readiness_packet(
            self,
            run_id=run_id,
            sink=sink,
            write=write,
    ...
    def positive_control_label_inventory(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_label_inventory(self.config, write=write)

    def positive_control_scenario_manifest(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_scenario_manifest(self.config, write=write)

    def positive_control_recognition_training_replay(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_recognition_training_replay(self.config, write=write)

    def positive_control_crop_workbench(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_crop_workbench(self.config, write=write)

    def positive_control_ocr_contact_drafts(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_ocr_contact_drafts(self.config, write=write)
    ...
    def positive_control_resume_readiness_gate(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_control_resume_readiness_gate(self.config, write=write)
    ...
    def positive_corpus_side_pair_evaluation(self, *, write: bool = True) -> dict[str, Any]:
        return build_positive_corpus_side_pair_evaluation(self.config, write=write)
    ...
    def _review_matrix_entry(
        ...
        *,
        ...
        crop_summary = redacted_crop_quality_summary(
            (artifacts.get("crop_quality") or {}).get("payload") or {}
        )
        recrop_summary = redacted_recrop_summary(
...
def _contact_crop_evidence_summary(assets: list[Any]) -> dict[str, Any]:
    crop_summaries: list[dict[str, Any]] = []
    recrop_summaries: list[dict[str, Any]] = []
    for asset in assets:
        if not isinstance(asset, dict):
            continue
        path = Path(str(asset.get("path_ref") or ""))
        if asset.get("asset_kind") == "crop_quality":
            payload = _read_json_file(path) or {}
            summary = redacted_crop_quality_summary(payload)
            summary["asset_id"] = asset.get("asset_id")
            summary["path_ref"] = str(path)
            crop_summaries.append(summary)
        if asset.get("asset_kind") == "recrop_proposals":
            payload = _read_json_file(path) or {}
            summary = redacted_recrop_summary(payload)
            summary["asset_id"] = asset.get("asset_id")
            summary["path_ref"] = str(path)
            recrop_summaries.append(summary)
    return {
        "crop_quality": {
            "asset_count": len(crop_summaries),
            "crop_count": sum(_int(summary.get("crop_count")) for summary in crop_summaries),
            "good_count": sum(_int(summary.get("good_count")) for summary in crop_summaries),
            "bad_count": sum(_int(summary.get("bad_count")) for summary in crop_summaries),
            "qr_only_count": sum(_int(summary.get("qr_only_count")) for summary in crop_summaries),
            "states": sorted({str(summary.get("state") or "unknown") for summary in crop_summaries}),
            "items": crop_summaries,
        },
        "recrop_proposals": {
            "asset_count": len(recrop_summaries),
            "proposal_count": sum(_int(summary.get("proposal_count")) for summary in recrop_summaries),
            "states": sorted({str(summary.get("state") or "unknown") for summary in recrop_summaries}),
            "items": recrop_summaries,
        },
    }
...
def _agent_review_reasons(
    *,
    ...
    recrop_summary: dict[str, Any],
    review_notes: list[dict[str, Any]],
    entry: dict[str, Any],
) -> list[str]:
    reasons: list[str] = []
    if _crop_quality_needs_review(crop_summary):
        reasons.append(f"crop_quality_{crop_summary.get('state') or 'needs_review'}")
    if _int(recrop_summary.get("proposal_count")) > 0:
        reasons.append("recrop_proposal_available")
    if _orientation_has_normalized_derivative(orientation_summary):
...
def _crop_quality_needs_review(crop_summary: dict[str, Any]) -> bool:
    return str(crop_summary.get("state") or "") in {"bad", "qr_only"} or _int(crop_summary.get("bad_count")) > 0
