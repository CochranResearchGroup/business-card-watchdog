...
def _render_contact_review_surface_text(payload: dict[str, object]) -> str:
    ...
    for row in rows if isinstance(rows, list) else []:
        if isinstance(row, dict):
            ...
            crop_quality = dict(row.get("crop_quality") or {})
            recrop_proposals = dict(row.get("recrop_proposals") or {})
            orientation_evidence = dict(row.get("orientation_evidence") or {})
            qr_evidence = dict(row.get("qr_evidence") or {})
            classifier_gate = dict(row.get("scanner_page_classifier_gate") or {})
            crop_acceptance = dict(row.get("crop_acceptance") or {})
            ocr_gate = dict(row.get("ocr_quality_gate") or {})
            app_intelligence = dict(row.get("app_intelligence_status") or {})
            lines.append(
                " - "
                ...
                f"crop_bad={crop_quality.get('bad_count', 0)} "
                f"recrops={recrop_proposals.get('proposal_count', 0)} "
                ...
                f"classifier={','.join(classifier_gate.get('classifier_training_states') or []) or 'missing'} "
                f"crop_gate={','.join(crop_acceptance.get('states') or []) or 'missing'} "
                f"ocr_gate={','.join(ocr_gate.get('states') or []) or 'missing'} "
                f"ai_requests={app_intelligence.get('request_count', 0)}"
            )
    return "\n".join(lines) + "\n"
...
def _render_negative_control_intake_text(payload: dict[str, object]) -> str:
    counts = dict(payload.get("counts") or {})
    commands = dict(payload.get("commands") or {})
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Negative-control intake: {payload.get('state')}",
        f"Label: {payload.get('operator_declared_label')}",
        ...
        f"Stored new: {counts.get('stored_new', 0)}",
        f"Deduped existing: {counts.get('deduped_existing', 0)}",
        f"Selection truncated: {payload.get('selection_truncated', False)}",
        "Observed: "
        f"files={payload.get('files_processed', 0)} "
        f"ocr={payload.get('ocr_attempted', 0)} "
        f"pdfs={payload.get('pdfs_rasterized', 0)} "
    ...
    if payload.get("manifest_path"):
        lines.append(f"Manifest path: {payload.get('manifest_path')}")
    if commands.get("negative_control_intake_preview"):
        lines.append(f"Preview: {commands.get('negative_control_intake_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def _render_positive_control_scenario_manifest_text(payload: dict[str, object]) -> str:
    coverage = dict(payload.get("coverage") or {})
    commands = dict(payload.get("commands") or {})
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Positive-control scenario manifest: {payload.get('state')}",
        f"Sources: {payload.get('source_count', 0)}",
        f"Scenarios: {payload.get('scenario_count', 0)}",
        f"Page-order swaps: {coverage.get('covers_page_order_swaps', False)}",
        f"Rotations: {coverage.get('covers_rotations', False)}",
        f"Omitted pages: {coverage.get('covers_omitted_pages', False)}",
        f"Duplicated pages: {coverage.get('covers_duplicated_pages', False)}",
        f"Front-only scans: {coverage.get('covers_front_only_scans', False)}",
        f"Back-only scans: {coverage.get('covers_back_only_scans', False)}",
        f"Mixed multi-card images: {coverage.get('covers_mixed_multi_card_images', False)}",
        f"Generated files: {payload.get('generated_file_count', 0)}",
        "Observed: "
        f"files={payload.get('files_inspected', 0)} "
        f"ocr={payload.get('ocr_attempted', 0)} "
    ...
    if payload.get("manifest_path"):
        lines.append(f"Manifest path: {payload.get('manifest_path')}")
    if commands.get("positive_control_scenario_manifest_preview"):
        lines.append(f"Preview: {commands.get('positive_control_scenario_manifest_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def _render_positive_control_recognition_training_replay_text(payload: dict[str, object]) -> str:
    before = dict(payload.get("before_counts") or {})
    after = dict(payload.get("after_counts") or {})
    improvement = dict(payload.get("improvement_summary") or {})
    commands = dict(payload.get("commands") or {})
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Positive-control recognition training replay: {payload.get('state')}",
        f"Sources: {payload.get('source_count', 0)}",
        f"Page/image cases: {payload.get('page_case_count', 0)}",
        f"Scenario plans: {payload.get('scenario_count', 0)}",
        f"False negatives before: {before.get('false_negative_cases', 0)}",
        f"False negatives after: {after.get('false_negative_cases', 0)}",
        "High-confidence known-positive negatives: "
        f"{improvement.get('high_confidence_negative_known_positive_before', 0)} -> "
        f"{improvement.get('high_confidence_negative_known_positive_after', 0)}",
        f"App Intelligence requests: {payload.get('app_intelligence_request_count', 0)}",
        f"Unknown thresholds changed: {payload.get('unknown_document_threshold_changed', False)}",
        "Observed: "
        f"ocr={payload.get('ocr_attempted', 0)} "
        f"pdfs={payload.get('pdfs_rasterized', 0)} "
        f"crops={payload.get('crops_created', 0)} "
    ...
    if payload.get("report_path"):
        lines.append(f"Report path: {payload.get('report_path')}")
    if commands.get("positive_control_recognition_training_replay_preview"):
        lines.append(f"Preview: {commands.get('positive_control_recognition_training_replay_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def _render_positive_control_crop_workbench_text(payload: dict[str, object]) -> str:
    counts = dict(payload.get("counts") or {})
    commands = dict(payload.get("commands") or {})
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Positive-control crop workbench: {payload.get('state')}",
        f"Sources: {payload.get('source_count', 0)}",
        f"Scenarios: {payload.get('scenario_count', 0)}",
        f"Processing units: {counts.get('processing_units', 0)}",
        f"PDF pages: {counts.get('pdf_page_units', 0)}",
        f"Crop candidates: {counts.get('crop_candidate_count', 0)}",
        f"Primary candidates: {counts.get('primary_crop_candidates', 0)}",
        f"Child candidates: {counts.get('child_crop_candidates', 0)}",
        f"Review requests: {counts.get('crop_review_requests', 0)}",
        f"Multi-card units with children: {counts.get('multi_card_units_with_children', 0)}",
        "Observed: "
        f"ocr={payload.get('ocr_attempted', 0)} "
        f"pdfs={payload.get('pdfs_rasterized', 0)} "
        f"crops={payload.get('crops_created', 0)} "
    ...
    if payload.get("report_path"):
        lines.append(f"Report path: {payload.get('report_path')}")
    if payload.get("candidate_artifact_dir"):
        lines.append(f"Candidate artifact dir: {payload.get('candidate_artifact_dir')}")
    if commands.get("positive_control_crop_workbench_preview"):
        lines.append(f"Preview: {commands.get('positive_control_crop_workbench_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def _render_positive_control_training_review_loop_text(payload: dict[str, object]) -> str:
    counts = dict(payload.get("counts") or {})
    commands = dict(payload.get("commands") or {})
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Positive-control training review loop: {payload.get('state')}",
        f"Review items: {payload.get('review_item_count', 0)}",
        f"Training candidates: {payload.get('training_candidate_count', 0)}",
        f"App Intelligence requests: {payload.get('app_intelligence_request_count', 0)}",
        f"Recognition items: {counts.get('recognition_items', 0)}",
        f"Scenario items: {counts.get('recognition_scenario_items', 0)}",
        f"Crop items: {counts.get('crop_items', 0)}",
        f"OCR items: {counts.get('ocr_items', 0)}",
        f"Side-pair items: {counts.get('side_pair_items', 0)}",
        f"Forbidden writes: routes={counts.get('routing_allowed_items', 0)} sinks={counts.get('sink_write_allowed_items', 0)} web={counts.get('public_web_allowed_items', 0)}",
        "Observed: "
        f"ocr={payload.get('ocr_attempted', 0)} "
        f"pdfs={payload.get('pdfs_rasterized', 0)} "
        f"crops={payload.get('crops_created', 0)} "
    ...
    if payload.get("report_path"):
        lines.append(f"Report path: {payload.get('report_path')}")
    if commands.get("positive_control_training_review_loop_preview"):
        lines.append(f"Preview: {commands.get('positive_control_training_review_loop_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def _render_positive_corpus_recognition_replay_text(payload: dict[str, object]) -> str:
    ...
    stop_conditions = payload.get("explicit_stop_conditions") or []
    lines = [
        f"Positive corpus recognition replay: {payload.get('state')}",
        f"Sources: {payload.get('source_count', 0)}",
        f"Pages replayed: {payload.get('page_count', 0)}",
        f"Images: {counts.get('images', 0)}",
        f"PDF documents: {counts.get('pdf_documents', 0)}",
    ...
    if payload.get("report_path"):
        lines.append(f"Report path: {payload.get('report_path')}")
    if commands.get("positive_corpus_recognition_replay_preview"):
        lines.append(f"Preview: {commands.get('positive_corpus_recognition_replay_preview')}")
    stop_rows = stop_conditions if isinstance(stop_conditions, list) else []
    lines.append(f"Stop conditions: {len(stop_rows)}")
    for condition in stop_rows:
        lines.append(f" - {condition}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    negative_control_intake = sub.add_parser("negative-control-intake")
    negative_control_intake.add_argument("--source", action="append", default=[])
    negative_control_intake.add_argument("--label", default="known_non_business_card")
    negative_control_intake.add_argument("--category", default="unclassified_non_card")
    negative_control_intake.add_argument("--include-glob", action="append", default=[])
    negative_control_intake.add_argument("--limit", type=int, default=None)
    ...
    positive_control_scenario_manifest = sub.add_parser("positive-control-scenario-manifest")
    positive_control_scenario_manifest.add_argument("--no-write", action="store_true")
    positive_control_scenario_manifest.add_argument("--json", action="store_true")

    positive_control_recognition_training_replay = sub.add_parser("positive-control-recognition-training-replay")
    positive_control_recognition_training_replay.add_argument("--no-write", action="store_true")
    positive_control_recognition_training_replay.add_argument("--json", action="store_true")

    positive_control_crop_workbench = sub.add_parser("positive-control-crop-workbench")
    ...
    positive_control_training_review_loop = sub.add_parser("positive-control-training-review-loop")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "negative-control-intake":
        payload = service.negative_control_intake(
            source_paths=list(args.source or []),
            label=str(args.label or "known_non_business_card"),
            category=str(args.category or "unclassified_non_card"),
            write=not args.no_write,
            include_globs=list(args.include_glob or []),
            limit=args.limit,
        )
    ...
    if args.command == "positive-control-scenario-manifest":
        payload = service.positive_control_scenario_manifest(write=not args.no_write)
        output = (
            json.dumps(payload, indent=2)
            if args.json
            else _render_positive_control_scenario_manifest_text(payload)
        )
        print(output, end="")
        return 0 if payload["state"] != "blocked" else 2

    if args.command == "positive-control-recognition-training-replay":
        payload = service.positive_control_recognition_training_replay(write=not args.no_write)
        output = (
            json.dumps(payload, indent=2)
            if args.json
            else _render_positive_control_recognition_training_replay_text(payload)
        )
        print(output, end="")
        return 0 if payload["state"] != "blocked" else 2
    ...
    if args.command == "positive-control-training-review-loop":
        payload = service.positive_control_training_review_loop(write=not args.no_write)
        output = (
            json.dumps(payload, indent=2)
            if args.json
    ...
    if args.command == "positive-corpus-recognition-replay":
        payload = service.positive_corpus_recognition_replay(write=not args.no_write)
        output = (
            json.dumps(payload, indent=2)
            if args.json
