# Plan 0025 | Child Selected Target Audit Guard
...
Create a no-live child selected-target audit preview that reports the current
child target packet, replacement requirements, and abandonment guard state.

## Non-Goals

- Do not create `selected_live_target.json`.
- Do not replace or abandon a live selected target.
...
- Service writes a child selected-target audit preview artifact from an existing
  child command-copy/checklist chain.
- Audit reports current child target identity, sink, operator, scope, and
  readiness state.
- Audit reports whether replacement would require abandonment, without
  performing abandonment.
- CLI exposes a child selected-target audit command.
- API exposes a child selected-target audit route.
- MCP exposes a child selected-target audit tool.
- Artifacts report zero live writes and zero network calls.
- Synthetic tests cover service, CLI, API, and MCP behavior.

## Execution Log

### Slice 0025-A | 2026-06-15 | Offline Child Audit Guard

Implemented:

- Add child selected-target audit preview service method.
...
- This slice writes only local child selected-target audit preview artifacts. It
  does not create, replace, or abandon selected targets, process private
