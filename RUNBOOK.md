...
- Added deterministic QR evidence artifacts for source images and candidate
  crops.
- Projected `qr_evidence` into the contact store and exposed redacted QR
  summaries in review bundles and contact review surfaces.
- Added `runs agent-review-loop <run-id> --limit <n> --dry-run --json` as a
  no-live planner for QR side, orientation, crop, and side-pairing blockers.
- Kept `--apply-safe` as a no-op boundary until deterministic handlers are
...
- `.venv/bin/python -m ruff check src/business_card_watchdog/qr_evidence.py src/business_card_watchdog/orchestrator.py src/business_card_watchdog/service.py src/business_card_watchdog/cli.py tests/test_qr_evidence.py tests/test_cli_surfaces.py`
  passed.
- Focused scanner dry-run `2026-06-23T02-27-30+00-00` completed from the
  cache-local 2026-06-20 scanner sequence with 14 jobs and 2 `needs_review`
  jobs.
- `runs agent-review-loop 2026-06-23T02-27-30+00-00 --limit 14 --dry-run --json`
  planned 2 QR follow-ups and 2 App Intelligence request candidates with
...
Executed Plan 0078 as the first Plan 0060 Milestone 7 Odollo routing slice.

Implemented:

- Added deterministic `providence_context` route matching for SoyLei/SABER
  context rules.
- Added route-rule Odollo tenant targets through `odollo_tenant`, `tenant`, or
  `target_tenant`.
...
- Threaded selected route targets into dry-run sink plans, sink payloads, and
  sink lookup plans.
- Added fixture tests for `soylei-prod` and `saber-prod` route targets.
...
Executed Plan 0082 as the first Plan 0060 Milestone 8 contact review surface
slice.
...
- Added audited normalized field corrections for projected contact rows.
- Added a composed contact review surface over assets, extraction attempts,
  enrichment attempts, routing decisions, sink attempts, review states, and
  mutation audit.
- Exposed the review surface and field-correction mutation through service,
  API, CLI, and MCP.
...
Executed Plan 0083 as the Plan 0060 Milestone 8 contact route-override mutation
slice.

Implemented:

- Added audited route override mutations for projected contact routing
  decisions.
- Updated route decisions in-place with selected sinks, selected sink state,
  selected target profile/tenant, route candidate state, and cleared readiness
  blockers.
- Preserved previous and new route state in `contact_mutations`.
- Exposed route override through service, API, CLI, and MCP.
...
Executed Plan 0086 as the Plan 0060 Milestone 8 contact review enrichment-merge
mutation slice.

Implemented:

- Added audited contact enrichment merge decisions against projected enrichment
  attempts.
- Applied only operator-approved supported fields to the contact row payload.
- Recorded previous/new values, approved fields, skipped proposals, and attempt
  id in `contact_mutations`.
- Exposed enrichment merge through service, API, CLI, and MCP.
...
- This slice did not call enrichment providers, run public-web search, process
  configured SyncThing/private watch inputs, perform live lookup/write/readback,
  or expose private image bytes.

## Turn 299 | 2026-06-22

Executed Plan 0087 as the Plan 0060 Milestone 8 contact review sink-approval
state mutation slice.

Implemented:

- Added audited contact sink approval state decisions against projected sink
  attempts.
- Persisted operator approval state into the sink attempt payload and recorded
  previous/new values in `contact_mutations`.
- Constrained approval states to `approved_for_lookup`,
  `approved_for_write_pilot`, `rejected`, and `needs_review`.
- Exposed sink approval through service, API, CLI, and MCP.
...
- This slice did not create selected-live-target artifacts, run live
