...
def test_manifest_has_process_tool() -> None:
    ...
    assert "business_card_watchdog_selected_target_command_copy_packet" in names
...
def test_mcp_contact_review_state_safe_loop_parity(tmp_path: Path) -> None:
    ...
    enrichment_merge = call_tool(
        "business_card_watchdog_contact_enrichment_merge",
        {
            "contact_id": contact_id,
            "operator": "mcp-test",
    ...
    sink_approval = call_tool(
        "business_card_watchdog_contact_sink_approval",
        {
            "contact_id": contact_id,
            "operator": "mcp-test",
    ...
    assert enrichment_merge["mutation"]["operator"] == "mcp-test"
    ...
    assert sink_approval["mutation"]["operator"] == "mcp-test"
...
def test_mcp_watch_backlog_preflight_redacts_private_source_paths(tmp_path: Path) -> None:
    ...
    payload = call_tool(
        "business_card_watchdog_watch_backlog_preflight",
        {"write": False},
        config=config,
    )
...
def test_mcp_dry_run_review_handoff_reports_safe_next_steps(tmp_path: Path, monkeypatch) -> None:
    source_dir = tmp_path / "cards"
    write_synthetic_image(source_dir / "card.png")
    config = AppConfig(
        config_path=tmp_path / "config.toml",
    ...
    payload = call_tool(
        "business_card_watchdog_dry_run_review_handoff",
        {"run_id": run_dir.name, "write": False},
        config=config,
    )
...
def test_mcp_review_route_readiness_reports_route_state(tmp_path: Path, monkeypatch) -> None:
    source_dir = tmp_path / "cards"
    write_synthetic_image(source_dir / "card.png")
    config = AppConfig(
        config_path=tmp_path / "config.toml",
    ...
    payload = call_tool(
        "business_card_watchdog_review_route_readiness",
        {"run_id": run_dir.name, "write": False},
        config=config,
    )
...
def test_mcp_selected_target_command_copy_packet_requires_ack(tmp_path: Path) -> None:
    config = AppConfig(
        config_path=tmp_path / "config.toml",
        data_dir=tmp_path / "data",
        sink=SinkConfig(google_contacts=True, dry_run=True),
    ...
    blocked = call_tool(
        "business_card_watchdog_selected_target_command_copy_packet",
        {
            "run_id": run_id,
            "operator": "tester",
            "sink": "google_contacts",
...
def test_mcp_watch_dry_run_selection_drill_exports_sample_output(tmp_path: Path) -> None:
    config = AppConfig(
        config_path=tmp_path / "config.toml",
        data_dir=tmp_path / "data",
        cache_dir=tmp_path / "cache",
    ...
    payload = call_tool("business_card_watchdog_watch_dry_run_selection_drill", {}, config=config)
...
def test_mcp_call_tool_dispatches_to_service(tmp_path: Path) -> None:
    ...
    assert operator_dashboard["commands"]["selected_target_approval_boundary"] == (
        f"runs selected-target-approval-boundary {run_id} "
        "--operator <operator> --response <operator-response> --no-write --json"
    )
    assert operator_dashboard["commands"]["selected_target_command_copy_packet"] == (
    ...
    assert operator_dashboard["mcp_tools"]["selected_target_approval_boundary"] == {
    ...
    assert operator_dashboard["mcp_tools"]["selected_target_command_copy_packet"] == {
