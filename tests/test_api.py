...
def test_api_health_status_runs_and_jobs(tmp_path: Path) -> None:
    ...
    assert operator_dashboard["api_routes"]["selected_target_approval_boundary"] == (
        f"POST /runs/{run_id}/selected-target-approval-boundary"
    )
    assert operator_dashboard["api_routes"]["selected_target_command_copy_packet"] == (
...
def test_api_run_review_route_readiness_reports_route_state(tmp_path: Path, monkeypatch) -> None:
    from business_card_watchdog.api import create_app
    ...
    payload = client.get(
        f"/runs/{run_dir.name}/review-route-readiness",
        params={"write": "false"},
    ).json()
